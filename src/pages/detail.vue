    ...
    <div class="wrapper">
      <div class="container clearfix">
        ...
        <div class="content">
          ...
          <p class="item-info">
            相机全新升级 / 960帧超慢动作 / 手持超级夜景 / 全球首款双频GPS / 骁龙845处理器 / 红<br />外人脸解锁
            / AI变焦双摄 / 三星 AMOLED 屏
          </p>
          <div class="delivery">小米自营</div>
          <div class="item-price">
            2599元<span class="del">{{ product.price }}元</span>
          </div>
          <div class="line"></div>
          <div class="item-addr">
            <i class="icon-loc"></i>
          ...
          <div class="item-total">
            <div class="phone-info clearfix">
              <div class="fl">
                {{ product.name }}
                {{ version === 1 ? '6GB+64GB 全网通 深灰色' : '4GB+64GB 移动4G' }}
              </div>
...
export default {
  ...
  methods: {
    ...
    async getProductInfo(productId) {
      try {
        this.product = await product.getProductInfo(productId)
      } catch (error) {
        console.log(error)
      }
    },
    async addCart() {
      try {
        const { res } = await cart.addCart(this.$route.params.id)
